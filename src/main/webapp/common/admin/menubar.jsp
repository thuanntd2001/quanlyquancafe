<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@include file="/common/taglib.jsp"%>

<div class="sidebar">
	<div class="logo_content">
		<div class="logo">
			<div class="logo_name">HIGHTLAND COFFEE</div>
		</div>

	</div>
	<ul class="nav_list">

		<li><a href="index.htm"> <i class="fas fa-table"></i> <span
				class="links_name">QUẢN LÝ NHÂN VIÊN</span>
		</a></li>
		<li><a href="admin-nhaphang.htm"> <i
				class="fas fa-cash-register"></i> <span class="links_name">QUẢN LÝ NHẬP HÀNG</span>
		</a></li>
		<li><a href="admin-qlban.htm"> <i
				class="fas fa-cash-register"></i> <span class="links_name">QUẢN LÝ BÀN</span>
		</a></li>
		<li><a href="admin-hoadon.htm"> <i
				class="fas fa-file-invoice-dollar"></i> <span class="links_name">QUẢN LÝ HÓA ĐƠN</span>
		</a></li>
		<li><a href="admin-qlthucdon.htm"> <i class="fas fa-coffee"></i>
				<span class="links_name">QUẢN LÝ THỰC ĐƠN</span>
		</a></li>
		<li><a href="admin-user.htm"> <i class="fas fa-user"></i> <span
				class="links_name">TÀI KHOẢN</span>
		</a></li>
		<li><a href="/CNPM/dang-nhap?action=logout"> <i
				class="fas fa-sign-out-alt"></i> <span class="links_name">ĐĂNG
					XUẤT</span>
		</a></li>
	</ul>
</div>

